...
class OkexApi {
	...
	async buy_limit_msg (instrument /* 币对 */, size /* 购买量 */, price /* 价格 */) {
		...
		this.log(`报警：币种为：${instrument}，买单失败。client_oid：${data.client_oid}。order_id：${data.order_id}。error_message：${data.error_message}`);
	}
	async sell_limit_msg (instrument /* 币对 */, size /* 购买量 */, price /* 价格 */) {
		...
		this.log(`报警：币种为：${instrument}，卖单失败。client_oid：${data.client_oid}。order_id：${data.order_id}。error_message：${data.error_message}`);
	}
